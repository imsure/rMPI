...
{
  int myrank, gridsize, num_iters, err_code, strip_size, iter, phy_rank;
  double **grid;
  ...
  PMPI_Comm_rank( MPI_COMM_WORLD, &phy_rank );
  ...
  if (myrank == 0) {
    end_time = MPI_Wtime();
    printf( "Physical rank: %d\tExecution time:%.3lf sec\n", phy_rank, end_time-start_time );
  }
